...
class Mediator:
    def __init__(
        ...
        event_sender: IDomainEventSender,
    ):
    ...
    async def _handle_create_task(self, command: CreateTaskCommand) -> Task:
        ...
        task_data = created_task.model_dump()
        task_data['task_id'] = task_data.pop('id')
        event = TaskCreatedEvent(**task_data)
    ...
    async def _handle_complete_task(self, command: CompleteTaskCommand) -> Task:
        ...
        task = await self._task_repository.get_by_id(str(command.task_id))
        if task:
    ...
    async def _handle_delete_task(self, command: DeleteTaskCommand):
        ...
        await self._task_repository.delete(str(command.task_id))
    ...
    async def _handle_get_task(self, query: GetTaskQuery) -> Task:
        ...
        return await self._task_repository.get_by_id(str(query.task_id))
